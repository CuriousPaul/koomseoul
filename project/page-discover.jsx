// Discovery page — split: filters | list | map

const { useState: useStateD, useMemo: useMemoD } = React;

function DiscoverPage({ lang, scheduleIds, toggleSchedule, prefilters }) {
  const [selectedTracks, setSelectedTracks] = useStateD(prefilters?.track ? [prefilters.track] : []);
  const [selectedNeighs, setSelectedNeighs] = useStateD([]);
  const [selectedAccess, setSelectedAccess] = useStateD([]);
  const [selectedDay, setSelectedDay] = useStateD("all");
  const [activeEvent, setActiveEvent] = useStateD(null);
  const [hoveredNeigh, setHoveredNeigh] = useStateD(null);
...
function SeoulMap({ eventCounts, hovered, setHovered, highlighted, onNeighClick }) {
  return (
    <div className="kw-map-canvas">
      {/* Stylized editorial line drawing of Seoul */}
      <svg viewBox="0 0 100 100" preserveAspectRatio="none">
        {/* Han River - sweeping curve through Seoul */}
...
Object.assign(window, { DiscoverPage, EventRow, SeoulMap, EventModal });

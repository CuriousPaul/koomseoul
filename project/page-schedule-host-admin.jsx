// Schedule Builder + Host Submission + Admin pages

const { useState: useStateP } = React;

// ---------------- SCHEDULE BUILDER ----------------
function SchedulePage({ scheduleIds, toggleSchedule, onNav }) {
  ...
  // Time slots in 2-hr buckets, 8am to midnight
            ...
            <div style={{marginTop: 48}}>
              ...
              {byDay.filter(d => d.events.length).map(d => (
                <div key={d.id} style={{marginBottom: 32}}>
                  ...
                  {d.events.map(e => (
                    <EventRow key={e.id} ev={e} active={false} inSchedule={true}
                              hasConflict={false} onClick={() => toggleSchedule(e.id)} />
                  ))}
                </div>
              ))}
            </div>
...
function AdminPage() {
  ...
  const [items, setItems] = useStateP(PENDING_SUBMISSIONS);
        ...
        <div className="kw-admin-head">
          ...
          <div style={{display:"flex", gap:8}}>
            {[
              ...
              <button key={t.k}
                      className={`day-tab ${tab === t.k ? "active" : ""}`}
                      onClick={() => setTab(t.k)}
                      style={{
                        padding: "8px 16px",
                        borderRadius: 4,
                        ...
                        cursor: "pointer",
                        background: tab === t.k ? "var(--ukf-ink)" : "transparent",
                        color: tab === t.k ? "#fff" : "var(--ukf-ink)",
                        border: "1px solid " + (tab === t.k ? "var(--ukf-ink)" : "var(--ukf-hairline)"),
                      }}>
...
Object.assign(window, { SchedulePage, HostPage, AdminPage });
